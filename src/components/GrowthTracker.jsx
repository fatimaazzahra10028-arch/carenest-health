...
const InputField = ({ label, icon: Icon, ...props }) => (
  <div className="space-y-2 flex-1">
    <label className="text-[11px] font-bold text-text-muted uppercase tracking-widest ml-2 transition-colors">
      {label}
    </label>
...
const GrowthTracker = ({ onBack, onSave, initialData }) => {
  ...
  const handleCalculate = (e) => {
    ...
    const bmi = (formData.weight / Math.pow(formData.height / 100, 2)).toFixed(
      1,
    );
    ...
    <div className="max-w-5xl mx-auto px-6 py-8 font-outfit bg-bg transition-colors duration-500 min-h-screen">
      <header className="flex items-center justify-between mb-12">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h2 className="text-3xl font-bold text-text-main font-kids transition-colors">
              Cek Pertumbuhan
            </h2>
            <p className="text-[11px] font-bold text-text-muted uppercase transition-colors">
              Medical Standar WHO 2026
            </p>
          </div>
        </div>
      </header>
          ...
          <form onSubmit={handleCalculate} className="space-y-6">
            ...
            <div className="flex gap-4">
              ...
              <div className="space-y-2 flex-1">
                ...
                <select
                  className="w-full h-[58px] px-6 bg-card rounded-[2rem] border-2 border-border-soft outline-none text-sm font-bold text-text-main shadow-sm transition-all"
        ...
        <div className="lg:col-span-7">
          <AnimatePresence mode="wait">
            {!result ? (
              <motion.div
                key="empty"
                className="h-full min-h-[450px] border-4 border-dashed border-border-soft rounded-[4rem] flex flex-col items-center justify-center p-12 text-center bg-card/30 transition-all"
              >
                <TrendUp size={40} className="text-primary/30 mb-4" />
                <h4 className="text-xl font-kids text-text-muted">
                  Siap Melihat Progres?
                </h4>
              </motion.div>
            ) : (
              <motion.div
                key="result"
                className="bg-card rounded-[4rem] p-10 text-text-main relative overflow-hidden shadow-3xl min-h-[450px] flex flex-col justify-between border border-border-soft transition-all duration-500"
              >
                <div className="absolute top-0 right-0 w-64 h-64 bg-primary/10 rounded-full blur-[100px] -mr-32 -mt-32" />
                <div className="relative z-10">
                  <h3 className="text-2xl font-kids mb-10 transition-colors">
                    Analisis {formData.name}
                  </h3>
                  <div className="grid md:grid-cols-2 gap-12 items-center">
                    <div className="relative">
                      <svg className="w-44 h-44 -rotate-90">
                        <circle
                          ...
                          cy="88"
                          r="80"
                          className="stroke-bg fill-none stroke-[12] transition-colors"
                        />
                        <motion.circle
                          cx="88"
                          cy="88"
                          ...
                          animate={{
                            strokeDashoffset:
                              502.6 - (502.6 * result.percentage) / 100,
                          }}
                          transition={{ duration: 2 }}
                        />
                      </svg>
                      <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <span className="text-4xl font-black text-text-main transition-colors">
                          {result.bmi}
                        </span>
                    ...
                    <div className="space-y-6">
                      <div
                        className={`inline-flex items-center gap-3 px-6 py-3 rounded-[1.5rem] ${result.bg} ${result.color}`}
                      >
                        <span className="text-lg font-black italic">
                          {result.status}
                        </span>
                      </div>
                      <p className="text-lg text-text-muted leading-relaxed transition-colors">
                        "{result.description}"
                      </p>
                    </div>
                  </div>
