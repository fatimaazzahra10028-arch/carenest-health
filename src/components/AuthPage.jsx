...
const AuthPage = ({ onSuccess, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const registeredUsers = JSON.parse(
      localStorage.getItem("moms_accounts") || "[]",
    );
    ...
    setTimeout(() => {
      if (isLogin) {
        ...
        if (!foundUser) {
          setErrors({
            email: "Akun tidak ditemukan. Silakan daftar dulu ya, Moms.",
          });
          setIsLoading(false);
        ...
        if (isExist) {
          setErrors({
            email: "Email ini sudah terdaftar. Silakan login saja.",
          });
          setIsLoading(false);
        } else {
          const newUser = {
    ...
    <div className="w-full max-w-4xl mx-auto bg-card rounded-[2.5rem] shadow-2xl flex flex-col md:flex-row border border-border-soft font-outfit overflow-hidden relative transition-all duration-500">
      <div className="hidden md:flex w-2/5 bg-bg/50 p-10 flex-col justify-between border-r border-border-soft relative overflow-hidden text-left transition-colors duration-500">
        ...
        <div className="flex items-center gap-2 relative z-10">
          ...
          <span className="font-kids text-2xl text-text-main font-bold tracking-tight transition-colors">
            MomsCare
          </span>
        </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="mt-5 text-text-muted text-sm leading-relaxed max-w-[200px] transition-colors">
                {isLogin
                  ? "Yuk, lanjut pantau tumbuh kembang si kecil hari ini."
                  : "Daftar sekarang untuk akses fitur lengkap MomsCare."}
              </p>
            </motion.div>
          </AnimatePresence>
        </div>
        ...
        <div className="flex gap-2 relative z-10">
          <div
            className={`h-1.5 rounded-full transition-all duration-500 ${isLogin ? "w-10 bg-primary" : "w-3 bg-border-soft"}`}
          />
          <div
            className={`h-1.5 rounded-full transition-all duration-500 ${!isLogin ? "w-10 bg-primary" : "w-3 bg-border-soft"}`}
          />
      ...
      <div className="w-full md:w-3/5 p-8 md:p-14 relative bg-card text-left transition-colors duration-500">
        ...
        <div className="mb-8">
          ...
          >
            ...
            <p className="text-text-muted text-sm mt-1 transition-colors">
              Silakan lengkapi detail di bawah ini ya, Moms.
            </p>
          </motion.div>
        </div>
...
const InputGroup = ({
  ...
  <div className="relative group text-left">
    <div
      className={`absolute left-4 top-[15px] transition-colors duration-300 ${error ? "text-red-400" : "text-text-muted group-focus-within:text-primary"}`}
    >
