...
const CategoryBlogSystem = ({
  ...
  const getArticleIcon = (categoryId) => {
    switch (categoryId) {
      case "neonatal":
        return <Baby size={32} weight="duotone" className="text-blue-500" />;
      case "health":
        return (
          <Stethoscope size={32} weight="duotone" className="text-red-500" />
        );
      case "vaccine":
        return <Flask size={32} weight="duotone" className="text-purple-500" />;
      case "nutrition":
        return (
          <AppleLogo size={32} weight="duotone" className="text-green-500" />
        );
      case "development":
        return <Brain size={32} weight="duotone" className="text-yellow-500" />;
      default:
        return <Article size={32} weight="duotone" className="text-primary" />;
    }
  };
        ...
        <div className="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-6 max-w-6xl mx-auto">
          {categories.map((cat) => (
            ...
            >
              ...
              >
                <div
                  className={`${cat.iconColor} transition-transform duration-300 group-hover:scale-110`}
                >
                  {cat.icon}
                </div>
              </motion.div>
              <div className="mt-4 text-center">
                <h3
                  className={`font-kids text-sm md:text-base transition-colors ${selectedCategory === cat.id ? "text-primary font-bold" : "text-text-main"}`}
                >
                  {cat.age}
                </h3>
                <p className="text-[9px] md:text-[10px] text-text-muted font-bold uppercase tracking-wider">
                  ({cat.range})
                </p>
              </div>
            </div>
