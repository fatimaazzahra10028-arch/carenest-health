...
const ImmunizationTracker = ({ childData, onBack }) => {
  ...
  const initialVaxList = [
    {
      ...
      age: 0,
      name: "Hepatitis B 1",
      desc: "Perlindungan Hati",
      detail:
        "Mencegah infeksi virus Hepatitis B yang dapat menyebabkan kerusakan hati kronis. Wajib diberikan dalam waktu 24 jam setelah bayi lahir.",
    },
    {
      id: 2,
      age: 1,
      name: "BCG & Polio 1",
      desc: "Cegah TBC & Lumpuh",
      detail:
        "Vaksin BCG melindungi paru-paru dari bakteri Tuberkulosis, sedangkan Polio mencegah risiko kelumpuhan permanen pada anak.",
    },
    {
      id: 3,
      age: 2,
      name: "DPT-HB-Hib 1",
      desc: "Vaksin Pentavalen",
      detail:
        "Memberikan perlindungan dari 5 penyakit sekaligus: Difteri, Tetanus, Pertusis (Batuk Rejan), Hepatitis B, dan Pneumonia/Meningitis (Hib).",
    },
    {
      id: 4,
      age: 3,
      name: "DPT-HB-Hib 2",
      desc: "Booster Kekebalan",
      detail:
        "Dosis kedua yang berfungsi untuk memperkuat memori sistem imun si kecil terhadap kuman difteri dan tetanus.",
    },
    {
      id: 5,
      age: 4,
      name: "Polio IPV 2",
      desc: "Proteksi Maksimal",
      detail:
        "Vaksin suntik (IPV) memberikan perlindungan yang jauh lebih kuat dan menyeluruh dibandingkan hanya menggunakan vaksin polio tetes.",
    },
    {
      id: 6,
      age: 9,
      name: "Campak / MR",
      desc: "Cegah Ruam & Radang",
      detail:
        "Vaksin ini sangat krusial untuk mencegah komplikasi berbahaya dari campak seperti radang otak, radang paru, dan kebutaan.",
    },
  ];
  ...
  const [vaccines, setVaccines] = useState(() => {
    ...
    return initialVaxList.map((vax) => ({
      ...vax,
      completed: childAge > vax.age,
    }));
  });
  ...
  const stats = useMemo(() => {
    ...
    return {
      count: completed,
      percent: Math.round((completed / vaccines.length) * 100),
    };
  }, [vaccines]);
    ...
    <div className="max-w-6xl mx-auto px-6 py-10 font-outfit relative bg-bg transition-colors duration-500 min-h-screen">
      ...
      <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8 mb-16">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-4xl font-kids text-text-main leading-tight transition-colors">
              Jadwal Imunisasi
            </h2>
      ...
      <div className="grid lg:grid-cols-12 gap-12">
        <aside className="lg:col-span-4 space-y-8">
          <div className="bg-card rounded-[3.5rem] p-10 text-text-main relative overflow-hidden shadow-3xl border border-border-soft">
            ...
            <div className="relative z-10">
              <h4 className="text-sm font-bold text-text-muted uppercase tracking-widest mb-2 font-outfit">
                Total Vaksinasi
              </h4>
          ...
          <div className="bg-card/50 backdrop-blur-md p-8 rounded-[3rem] border border-border-soft flex flex-col items-center text-center shadow-sm group transition-all">
            ...
            <h4 className="font-bold text-text-main mb-2 font-kids text-xl transition-colors">
              Lokasi Terdekat
            </h4>
            <p className="text-xs text-text-muted leading-relaxed mb-6 px-4 transition-colors">
              Cari Puskesmas atau Rumah Sakit penyedia vaksinasi di sekitarmu.
            </p>
            <button
              onClick={() =>
                window.open(
                  `https://www.google.com/maps/search/Rumah+Sakit+atau+Puskesmas+terdekat`,
                  "_blank",
                )
              }
        ...
        <main className="lg:col-span-8 space-y-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-kids text-text-main px-4 transition-colors">
              Timeline Kesehatan
            </h3>
            <span className="text-[10px] font-black text-text-muted uppercase border-b-2 border-primary/20 pb-1 transition-colors">
              Standar IDAI 2026
            </span>
          </div>
                ...
                <motion.div
                  ...
                  className={`group relative flex items-center gap-6 p-6 rounded-[2.5rem] border-2 transition-all duration-500 ${
                    vax.completed
                      ...
                      : isDue
                        ? "bg-card border-primary shadow-xl ring-8 ring-primary/5"
                ...
                >
                  <motion.button
                    ...
                    className={`w-16 h-16 rounded-[1.8rem] flex items-center justify-center shrink-0 transition-all ${
                      vax.completed
                        ? "bg-secondary text-white rotate-[360deg]"
                        : isDue
                  ...
                  <div
                    ...
                    onClick={() => !isLocked && setSelectedVax(vax)}
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span
                        className={`text-[10px] font-bold uppercase ${vax.completed ? "text-secondary" : "text-text-muted"}`}
                      >
                        Bulan ke-{vax.age}
                      </span>
                    </div>
                    <h5 className="text-lg font-bold text-text-main font-kids transition-colors">
                      {vax.name}
                    </h5>
                    <p className="text-xs text-text-muted font-medium transition-colors">
                      {vax.desc}
                    </p>
                  </div>
                  ...
                  <CaretRight
                    size={20}
                    className="text-text-muted/30 group-hover:text-primary transition-colors"
                  />
                </motion.div>
              );
            })}
        ...
        {selectedVax && (
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-6">
            ...
            >
              ...
              <h4 className="text-3xl font-kids text-text-main mb-4 transition-colors">
                {selectedVax.name}
              </h4>
