...
import { CaretLeft, CaretRight, Clock, User, ArrowUpRight } from '@phosphor-icons/react';
...
const TopicPage = ({ categoryId, onBack, onArticleClick }) => {
  ...
  const currentArticles = topicArticles.slice((currentPage - 1) * articlesPerPage, currentPage * articlesPerPage);
  ...
  return (
    <div className="min-h-screen bg-[#F9FBFC] font-outfit pb-20">
      ...
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 py-3">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <div className="flex flex-col items-end">
            ...
            <span className="font-kids text-lg text-slate-800 leading-none">{categoryInfo.age}</span>
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-6xl mx-auto px-6 mt-10">
        <header className="mb-10">
          <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}>
            <h1 className="text-3xl md:text-4xl font-kids text-slate-900 mb-2">
              Edukasi <span className="text-primary italic">{categoryInfo.age}</span>
            </h1>
        ...
        {/* Bento Grid dengan Ukuran Proporsional (Compact) */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
          <AnimatePresence mode="wait">
            {currentArticles.map((article, index) => {
              // Menentukan ukuran kolom (Span 7 untuk yang utama, Span 5 untuk pendamping)
              ...
              return (
                ...
                >
                  <div className="h-full bg-white border border-slate-100 rounded-[2rem] p-3 shadow-sm hover:shadow-xl hover:shadow-primary/5 transition-all duration-500 flex flex-col">
                    {/* Image Area - Dikecilkan */}
                    <div className={`relative overflow-hidden rounded-[1.6rem] ${isLarge ? 'h-52 md:h-64' : 'h-44'}`}>
                      <img 
                        src={article.img} 
                    ...
                    <div className="p-4 flex flex-col flex-grow">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">Oleh {article.author}</span>
                      </div>

                      <h3 className={`font-kids text-slate-800 group-hover:text-primary transition-colors leading-tight mb-2 italic ${isLarge ? 'text-xl md:text-2xl' : 'text-lg'}`}>
                        {article.title}
                      </h3>
                      ...
                      <div className="mt-auto flex items-center justify-between pt-3 border-t border-slate-50">
                        <span className="text-[10px] font-bold text-primary uppercase">Baca Selengkapnya</span>
                ...
                {[...Array(totalPages)].map((_, i) => (
                  <button
                    ...
                    className={`w-9 h-9 rounded-xl text-xs font-bold transition-all ${
                      currentPage === i + 1 ? 'bg-primary text-white shadow-md shadow-primary/20' : 'text-slate-400 hover:bg-slate-50'
                    }`}
                  >
                    {i + 1}
                  </button>
